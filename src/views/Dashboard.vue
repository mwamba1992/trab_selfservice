...
const kpis = computed(() => [
  {
    key: 'notices',
    label: t('dashboard.noticesFiled'),
    value: stats.value.notices,
    icon: 'pi pi-file',
    color: '#1B6B3D',
    bg: 'rgba(27,107,61,0.08)',
    to: '/notices',
  },
  {
    key: 'appeals',
    label: t('dashboard.appeals'),
    value: stats.value.appeals,
...
<template>
  <div>
    ...
    <div v-if="failed" class="note note-error" role="alert">
      <i class="pi pi-exclamation-triangle"></i>
      <span class="flex-1">{{ t('common.loadFailed') }}</span>
      <Button :label="t('common.retry')" size="small" outlined @click="load" />
    </div>

    <router-link v-if="stats.unreadNotifications > 0" to="/notifications" class="note note-info">
    ...
    <h3 class="text-sm font-semibold mb-3 text-label">{{ t('dashboard.quickActions') }}</h3>
